...
describe('ak-button/default-behaviour', () => {
  ...
  it('should set accessibility attributes', () => {
    ...
    expect(mount(<Button ariaHaspopup />).find('button[aria-haspopup=true]').length).toBe(1);
    expect(mount(<Button ariaExpanded />).find('button[aria-expanded=true]').length).toBe(1);
    expect(mount(<Button ariaControls="test" />).find('button[aria-controls="test"]').length).toBe(1);
    expect(mount(<Button id="test" />).find('button[id="test"]').length).toBe(1);
  });
});
