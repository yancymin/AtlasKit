...
export const colorWithAlphaSame = (a: ColorWithAlpha, b: ColorWithAlpha): boolean => {
  return a.red === b.red &&
         a.green === b.green &&
         a.blue === b.blue &&
         a.alpha === b.alpha;
...
function splitText<T>(text: string, charCodeHandler: (code: number) => T,
                      surrogatePairHandler: (high: number, low: number) => T): Array<T> {
  const result: Array<T> = [];

  for (let i = 0; i < text.length; ++i) {
    const current = text.charCodeAt(i);

    if (current >= 0xD800 && current <= 0xDBFF && i < text.length - 1) {  // high surrogate
      const next = text.charCodeAt(i + 1);
      ++i;

      if (next >= 0xDC00 && next <= 0xDFFF) {  // low surrogate
        result.push(surrogatePairHandler(current, next));
      } else {  // the string is broken
...
export function adjustSize<T>(elements: Array<T>, requiredSize: number, creator: () => T, deleter: (element: T) => void): void {
