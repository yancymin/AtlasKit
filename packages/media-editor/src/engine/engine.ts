...
export class Engine {
  ...
  private subscribeToComponentsSignals(): void {
    ...
    toolbar.colorChanged.listen((color) => this.veCall('update color', ve => ve.setColor(color)));
  ...
  private initModule(): void {
    ...
    this.module.handleShapeParametersChanged = (red: number, green: number, blue: number,
                                                lineWidth: number, addShadow: boolean) => {
      toolbar.updateByCore({color: {red, green, blue}, lineWidth, addShadow});
    };

    this.module.handleTextInputStarted = () => { keyboardInput.startInput(); };
    this.module.handleTextInputEnded = () => { keyboardInput.endInput(); };

    const typesetter = new BrowserTypesetter({
      gl, module: this.module, ...keyboardInput
    });
  ...
  private createVeEngine(): void {
    ...
    this.resourceManager.addCustom(() => { this.ve.delete(); });
  ...
  private veCall(description: string, method: (ve: Core.VeEngine) => boolean): void {
    if (!method(this.ve)) {
  ...
  private toTextCommand(inputCommand: InputCommand): Core.VeTextInputCommand {
    const {CompleteInput, NewLine, Backspace, Delete,
           MoveCursorLeft, MoveCursorRight, MoveCursorUp, MoveCursorDown} = this.module.VeTextInputCommand;
