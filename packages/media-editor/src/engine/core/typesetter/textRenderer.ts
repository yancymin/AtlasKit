...
type TextRenderer = (text: string, context: CanvasRenderingContext2D,
                     measures: TextMeasures, strokeThickness: number) => void;

// Renders one line of the text. Pushes new fragments to the 'fragments' array
export const renderText = (fragments: Array<Fragment>, config: TextRenderingConfig): boolean => {
  ...
  const generationCompleted = normalTextures.length === strokeTextures.length &&
...
const setCanvasSize = (canvas: HTMLCanvasElement, measures: TextMeasures): void => {
  canvas.width = measures.width;
  canvas.height = measures.height;
};

// Sets canvas diretion, returns the previous direction
const setCanvasDir = (canvas: HTMLCanvasElement, textDirection: TextDirection): string => {
...
const generateTextures = (config: TextRenderingConfig, measures: TextMeasures, renderer: TextRenderer): Array<GeneratedTexture> => {
...
const renderNormalText = (text: string, context: CanvasRenderingContext2D, measures: TextMeasures): void => {
  context.fillStyle = '#FFFFFF';
  context.fillText(text, measures.x, measures.y);
};

const renderStroke = (text: string, context: CanvasRenderingContext2D, measures: TextMeasures, strokeThickness: number): void => {
...
const clearCanvas = (context: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void => {
...
const sliceCanvasToTextures = (context: CanvasRenderingContext2D, gl: WebGLRenderingContext, measures: TextMeasures): Array<GeneratedTexture> => {
...
const createTexture = (context: CanvasRenderingContext2D, gl: WebGLRenderingContext,
                       xstart: number, xend: number, height: number): WebGLTexture | null => {
...
const deleteTextures = (gl: WebGLRenderingContext, textures: Array<GeneratedTexture>): void => {
