...
export class Typeset implements Core.TypesetInterop {
  ...
  unload(): void {  // might be called multiple times
    this.destroy();
  }
  ...
  update(text: string, textLength: number, direction: string, fontSize: number, cursorArray: number): boolean {
    if (this.isContextLost) {  // sanity check because we manipulate textures during update
      return false;
    }

    this.fontMetrics = this.config.fontInfo.getFontMetrics(fontSize);

    return this.updateParagraphs(text, direction as TextDirection, fontSize) &&
  ...
  bindNormal(fragmentIndex: number): boolean {
    return this.bindFragmentTexture(fragmentIndex, fragment => fragment.bindNormal());
  }

  bindStroke(fragmentIndex: number): boolean {
    return this.bindFragmentTexture(fragmentIndex, fragment => fragment.bindStroke());
  }
  ...
  getYBase(fragmentIndex: number): number {
    return this.getFragmentCoordinate(fragmentIndex, (pos, yline) => pos.ybase + yline);
  }
  ...
  getYOpposite(fragmentIndex: number): number {
    return this.getFragmentCoordinate(fragmentIndex, (pos, yline) => pos.yopposite + yline);
  }
  ...
  private bindFragmentTexture(index: number, bindMethod: (fragment: Fragment) => boolean): boolean {
    return bindMethod(this.fragments[index].fragment);
  }

  private getFragmentCoordinate(index: number, getter: (position: FragmentPosition, yline: number) => number): number {
    const {fragment, yline} = this.fragments[index];
    return getter(fragment.position, yline);
  }

  private updateParagraphs(text: string, direction: TextDirection, fontSize: number): boolean {
    ...
    adjustSize<Paragraph>(
      this.paragraphs, paragraphTexts.length,
      () => {
        return new Paragraph(this.config);
      },
      (paragraph: Paragraph) => {  // paragraphs contain textures and must be released explicitly
  ...
  private collectParagraphData(cursorArray: number, cursorPosCount: number): boolean {
  ...
  private collectCursorPositions(cursorArray: number, cursorPosCount: number): boolean {
    ...
    this.paragraphs.forEach((par, parIndex) => {
      ...
      par.textCursorPositions.forEach((x) => {
        if (index < count - 1) {  // We must check the limits for not to destroy heap data
