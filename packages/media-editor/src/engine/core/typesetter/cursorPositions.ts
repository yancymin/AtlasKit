...
export const getCursorPositions = (text: string, direction: TextDirection,
                                   textHelperDiv: HTMLDivElement): Array<number> => {
  if (text.length === 0) {
    return [0];
  }
  ...
  const cursors = detectCursorPositions(getUtf32CodeUnits(text), direction, rootSpan);
...
const detectCursorPositions = (text: Text, direction: TextDirection, rootSpan: HTMLSpanElement): Array<number> => {
...
const initCursorsArray = (length: number, value = 0) => Array.apply(null, {length}).map(() => value);


// Extracting groups from the text

const getGroups = (text: Text, baseDirection: TextDirection, rootSpan: HTMLSpanElement): Array<Group> => {
...
const extractBaseGroups = (text: Text, direction: TextDirection, rootSpan: HTMLSpanElement): Array<GroupBase> => {
  ...
  text.forEach((character, index) => {
    if (isWhiteSpace(character)) {  // The current character is a whitespace. It forms one group.
      ...
      if (currentText.length === 0) {  // We only start to extract a group
        ...
        } else {  // we continue pushing characters into the current group
          currentText.push(character);
        }
...
export const getDirection = (index: number, spans: Array<HTMLSpanElement>): TextDirection | undefined => {
  ...
  if (isClose(previousRect.right, currentRect.left)) {  //  | previous || current |
    return 'ltr';
  } else if (isClose(previousRect.left, currentRect.right)) {  //  | current || previous |
    return 'rtl';
  } else {
    return undefined;
  }
...
export const getGroupStartIndices = (baseGroups: Array<GroupBase>): Array<number> => {
  let index = 0;
...
export const getGroupXMinMax = (baseGroups: Array<GroupBase>, direction: TextDirection, rootSpan: HTMLSpanElement): Array<GroupMinMax> => {
  ...
  const xorigin = (direction === 'rtl') ? parentSpanRect.right : parentSpanRect.left;
...
export const createSpansForCharacters = (text: Text, parent: HTMLSpanElement): Array<HTMLSpanElement> => {
  return text
    .map((character) => {
...
// Wrap each group into a span
export const createSpansForGroups = (baseGroups: Array<GroupBase>, parent: HTMLSpanElement): Array<HTMLSpanElement> => {
  return baseGroups
    .map((group) => {
      const span = document.createElement('span');
...
const setInternalCursorPositions = (group: Group, rootSpan: HTMLSpanElement, cursors: Array<number>): void => {
  ...
  if (group.direction === 'rtl') {
    putInternalPositions(
      group.text, span, cursors, group.startIndex,
...
export const putInternalPositions = (text: Text, span: HTMLSpanElement, cursors: Array<number>, startIndex: number,
                                     posCalculator: (spanWidth: number) => number,
                                     posLimiter: (position: number) => number): void => {
