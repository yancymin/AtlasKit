...
export type TimerStarter = (callback: TimerCallback, msecInterval: number) => TimerHandle;
...
class ActiveTimer {
  ...
  constructor(callback: TimerCallback, msecInterval: number,
              starter: TimerStarter, private stopper: TimerStopper) {
...
export class TimerFactory implements Core.TimerFactoryInterop {
  ...
  constructor(private onTick: TimerTickHandler,
  ...
  private static defaultStarter(callback: TimerCallback, msecInterval: number): TimerHandle {
